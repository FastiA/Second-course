...
int linSearch(TrapArray* anArray, int key)
{
	int* result = (int*)malloc(sizeof(int) * anArray->count);
	int j = 0;
	if (NULL != anArray)
	{
		for (int i = 0; i < anArray->number; i++)
		{
			if (TrapezeSquare(anArray->traps[i]) == key)
			{
				result[j] = i;
				j++;
			}
